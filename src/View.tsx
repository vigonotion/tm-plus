...
import { Box, Line, Segment, Segments, Text } from "@react-three/drei";
...
const indices = [
  ...
  2,
  2,
  0, // Edges of the square
  1,
  3, // Edge of the triangle
];

function Widget({ width, height }: { width: number; height: number }) {
  const geometry = new BoxGeometry(1, 1, 1);

  return (
    <Segments lineWidth={10}>
      <Segment start={[0, 0, 0]} end={[0, 100, 0]} color="blue" />
      <Segment start={[0, 0, 0]} end={[100, 0, 0]} color="orange" />
      <Segment start={[0, 100, 0]} end={[100, 100, 0]} color="red" />
      <Segment start={[100, 0, 0]} end={[100, 100, 0]} color="green" />
    </Segments>
  );
}

const SizedScene = React.memo(
